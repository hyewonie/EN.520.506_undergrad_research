...
class Soft_WL_Subtree_NodeLevel(object):
    ...
    def graph_convolution(self, adj, x):
        """
        Perform graph convolution on the node features using GPU via CuPy.
        Parameters
        ----------
        adj : scipy sparse matrix or numpy array, shape = [n_nodes, n_nodes]
        ...
        try:
            # Verify that adj_cp is a CuPy sparse CSR matrix
            print(f"Type of adj in graph_convolution: {type(adj)}")
            
            # Convert node features to CuPy array
            x_cp = cp.asarray(x, dtype=cp.float32)
            print(f"Node features are on device: {x_cp.device}")

            # Perform graph convolution iterations
            for i in range(self.n_iter):
                x_cp = adj.dot(x_cp)
                # print(f"Iteration {i+1}: Feature matrix device after convolution: {x_cp.device}")

            # Convert back to NumPy array
            return cp.asnumpy(x_cp)
        except cp.cuda.memory.OutOfMemoryError:
            print("CUDA Out of Memory. Switching to CPU-based graph convolution.")
            adj = adj.get()  # Convert back to SciPy CSR matrix
            ...
            print(f"An unexpected error occurred during graph convolution: {e}")
            raise
